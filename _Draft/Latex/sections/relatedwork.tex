...
\label{sec:related_levelgeneration}

本節紀載了至今的關卡生成框架與方法，同時收錄數款應用了關卡生成技術之遊戲。而參考的文獻主要分為兩種類型，分別為~\ref{ssec:related_missionspace} 小節的程序化生成任務內容與~\ref{ssec:related_mapsketches} 小節的程序化遊戲物件擺放。

\subsection{Mission/Space 框架}
\label{ssec:related_missionspace}
...
\subsection{Map Sketches 與 Segments 的演化}
\label{ssec:related_mapsketches}
...
\label{ssec:related_unexplored}

Unexplored 是由 ~\ref{ssec:related_missionspace} 小節提及之框架作者 Joris Dormans 所製作，Joris Dormans 基於 Mission/Space 框架並提出改良的即時型戰鬥 roguelike 遊戲（若再細分可歸類於 roguelite 遊戲類型）。在市面上普遍的 roguelike 遊戲中，最常見的關卡生成策略即是視地下城玩家起點位置為樹之根，再以此根持續添加生成或預先設計的
