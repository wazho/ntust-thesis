...
「關聯權重 (relative weight) 」與「數量限制」提供 \textit{Dungeon Generator} 在設計任務規則時更多地彈性，關卡設計師能夠針調整特定的任務規則被套用的機率與套用的次數。表~\ref{tbl:mission-rules-relative-weight} 是基於任務規則表（表~\ref{tbl:missiongrammars-rules-linear-example} - ~\ref{tbl:missiongrammars-rules-nonlinear-example3}）所建立的關聯權重表，關聯權重是一自然常數，當改寫系統進行時，會依照關聯權重與選定方法挑選出一任務規則。於下一小節中，關於關聯權重有詳細的解釋。
...
\begin{algorithm}[!htb]
    ...
    \begin{algorithmic}[1]
        ...
        \Ensure
            ...
            \State {}
            \Function{RewriteSystem1}{$root$, $rules$, $seed$}
                \State {$matchedRule$ = FindMatchs($root$, $rules$)}       \Comment{演算法~\ref{alg:algorithm-matchedrule} 之方法，尋找匹配規則}
                \If {$matchedRule$ is found from $root$ and $root$ doesn't be explored}
                    \State {RemoveConnections($root$, $matchedRule$)}      \Comment{圖~\ref{fig:rewrite-system-i-flow} b，移除連接線}
                    \State {ReplaceNodes($root$, $matchedRule$)}           \Comment{圖~\ref{fig:rewrite-system-i-flow} c，替換等價節點}
                    \State {AppendNodes($root$, $matchedRule$)}            \Comment{圖~\ref{fig:rewrite-system-i-flow} d，補充額外節點}
                    \State {ReAddConnections($root$, $matchedRule$)}       \Comment{圖~\ref{fig:rewrite-system-i-flow} e，重建連接線}
                    \State {RevokeIndexes($root$, $matchedRule$)}          \Comment{圖~\ref{fig:rewrite-system-i-flow} f，移除索引值}
                \EndIf
                \ForAll {$child\in children$ of $root$}:
                    \State {RewriteSystem1($child$, $rules$, $seed$)}      \Comment{遞迴式，跳往 Line $3$}
                \EndFor
                \State {\Return {$root$}}
            \EndFunction
    \end{algorithmic}
