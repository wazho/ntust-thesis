...
圖~\ref{fig:mission-to-space-instruction-flow} a 使用第~\ref{ssec:method-missiongrammars-rules} 小節的任務規則表（表~\ref{tbl:missiongrammars-rules-linear-example}-~\ref{tbl:missiongrammars-rules-nonlinear-example3}）來生成任務圖。本階段我們依照演算法~\ref{alg:algorithm-mission-to-space-rewritesystem}，並結合~\ref{tbl:mission-to-space-instruction} 建造方式表，將任務圖轉換為遊戲空間。圖~\ref{fig:mission-to-space-instruction-flow} b 中，
