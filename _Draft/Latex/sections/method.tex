...
\begin{table}[!htb]
  ...
  \begin{tabular}{
    ...
    ? (any)
      ...
      & 此結點無法修改，於~\ref{ssec:method-missiongrammars-rules} 小節 - 建立任務規則中，任一節點能夠被判別為任何節點，促使其同構。
      \\\hline
  \end{tabular}
\end{table}
...
\begin{longtable}{
  ...
  S (Start)
    & 起始節點
    & \missionalphabetnode{nt-start}{8mm}
    & 任務圖進入改寫階段時，系統預設的根節點。
    & 起始節點 \\\hline
  en (entrance)
    & 起點
    & \missionalphabetnode{t-entrance}{8mm}
    & 玩家於遊戲關卡開始時的起始位置。
    & \multirow{8.5}{*}{ 主線任務 } \\\cline{1-4}
    % 8.5 is special to centering.
  go (goal)
    & 終點
    & \missionalphabetnode{t-goal}{8mm}
    & 玩家抵達此位置時遊戲將結束。
    &  \\\cline{1-4}
  boss (boss)
    & 魔王房
    & \missionalphabetnode{t-boss}{8mm}
    & 房間中存在著較強大的怪物。
    &  \\\cline{1-4}
  NM (Normal)
    & 一般房
    & \missionalphabetnode{nt-normal}{8mm}
    & 一般房間，藉由其它規則替換成其它房間或事件。
    &  \\\cline{1-4}
  nm (normal)
    & 一般房
    & \missionalphabetnode{t-normal}{8mm}
    & 遊戲過程中經過的通道，經常出現在遊戲關卡中。
    &  \\\hline
  bomb (bomb)
    & 使用炸彈
    & \missionalphabetnode{t-bomb}{8mm}
