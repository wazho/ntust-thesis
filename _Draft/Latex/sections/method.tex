...
方程式~\ref{eq:fitnesses-all} 中定義房間容器（個體）的品質，根據本小節提及的數種遊玩特徵指標與第~\ref{ssec:method-segments-balancefitness} 小節將提到的平衡指標計算得分 ($f_{i}$)，得分是由各項指標代表一則適應性函數計算而來，依照各函數得分與其權重值 ($w_{i}$) 加權後加總得到 $f_{all}$，其中 $M$ 為權重非 0 的適應性函數之數量。

\begin{equation}
    \label{eq:fitnesses-all}
    f_{all} = \sum_{i=1}^{M} (f_{i} \times w_{i})
\end{equation}
...
其餘未被定義的房型，像是入口、出口、一般通道 ... 等，因設計需求將不配置任何遊戲物件，將不參與相關演化工作。
